...
_ALLOWED_KINDS = {
    "service_matcher", "active_probe", "os_fingerprint", "udp_probe",
    "product_vocab", "registry", "web_fingerprint", "device_fingerprint",
    "port_registry", "product_record", "product_alias", "cpe_record",
}
...
_ALLOWED_EVIDENCE_DIMENSIONS = {
    "banner", "http_header", "http_cookie", "http_html", "http_script",
    "http_url", "http_meta", "http_js", "http_dom", "http_css",
    "tls", "protocol", "device", "os",
}
...
def _semantic_payload(record: CanonicalRecord) -> dict[str, object]:
    return {
        "kind": record.kind, "transport": record.transport,
        "address_family": record.address_family, "probe_id": record.probe_id,
        "probe_payload_ref": record.probe_payload_ref, "matcher_type": record.matcher_type,
        "matcher_expression": record.matcher_expression, "service": record.service,
        "vendor": record.vendor, "product": record.product, "version": record.version,
        "version_info": record.version_info, "os_family": record.os_family,
        "os_generation": record.os_generation, "device_type": record.device_type,
        "cpe": _normalized_strings(record.cpe), "ports": sorted(set(record.ports)),
        ...
        "probe_priority": record.probe_priority, "probe_timeout_ms": record.probe_timeout_ms,
        "fallback_probe_ids": list(record.fallback_probe_ids),
        "match_strength": record.match_strength, "extra_template": record.extra_template,
        "hostname_template": record.hostname_template, "tunnel_template": record.tunnel_template,
        "protocol_hint": record.protocol_hint, "max_response_bytes": record.max_response_bytes,
        ...
        "probe_order": record.probe_order, "rule_order": record.rule_order,
        "evidence_dimension": record.evidence_dimension, "alias": record.alias,
    }


def stable_record_id(record: CanonicalRecord) -> str:
    encoded = json.dumps(_semantic_payload(record), sort_keys=True, separators=(",", ":"), ensure_ascii=False).encode("utf-8")
...
def _valid_payload_hex(value: object) -> bool:
    return isinstance(value, str) and len(value) % 2 == 0 and all(character in _HEX_CHARS for character in value)
...
def _validate_os_features(values: object) -> list[str]:
    ...
    keys: list[str] = []
    for item in values:
        if not isinstance(item, tuple) or len(item) != 2 or not isinstance(item[0], str) or not isinstance(item[1], str) or not item[0].strip():
...
def validate_record(record: CanonicalRecord, sources: Mapping[str, SourcePolicy]) -> list[str]:
    errors: list[str] = []
    ...
    if record.confidence is not None and (type(record.confidence) not in (int, float) or not 0.0 <= float(record.confidence) <= 1.0):
    ...
    if any(type(port) is not int or not 0 <= port <= 65535 for port in record.ports):
    ...
    if record.probe_priority is not None and (type(record.probe_priority) is not int or record.probe_priority < 0):
        errors.append("probe_priority must be a non-negative integer or null")
    if record.probe_timeout_ms is not None and (type(record.probe_timeout_ms) is not int or record.probe_timeout_ms <= 0):
    ...
    elif any(not isinstance(value, str) or not value.strip() for value in record.fallback_probe_ids) or len(record.fallback_probe_ids) != len(set(record.fallback_probe_ids)):
    ...
    if record.protocol_hint is not None and (not isinstance(record.protocol_hint, str) or not record.protocol_hint.strip()):
        errors.append("protocol_hint must be a non-empty string or null")
    if record.max_response_bytes is not None and (type(record.max_response_bytes) is not int or record.max_response_bytes <= 0):
        errors.append("max_response_bytes must be a positive integer or null")
    if record.specificity is not None and (type(record.specificity) is not int or record.specificity < 0):
    ...
    for order_name, order_value in (("probe_order", record.probe_order), ("rule_order", record.rule_order)):
    ...
    if record.kind == "service_matcher":
        if not record.probe_id: errors.append("service_matcher requires probe_id")
        if not record.matcher_type: errors.append("service_matcher requires matcher_type")
        if not record.matcher_expression: errors.append("service_matcher requires matcher_expression")
        if not record.service: errors.append("service_matcher requires service")
        if record.match_strength not in _ALLOWED_MATCH_STRENGTHS: errors.append("service_matcher requires match_strength hard or soft")
    elif record.kind == "active_probe":
        if not record.probe_id: errors.append("active_probe requires probe_id")
        if record.probe_payload_hex is None: errors.append("active_probe requires probe_payload_hex")
        if record.probe_timeout_ms is None: errors.append("active_probe requires probe_timeout_ms")
    elif record.kind == "udp_probe":
        if not record.probe_id: errors.append("udp_probe requires probe_id")
        if len(record.ports) != 1: errors.append("udp_probe requires exactly one port")
        if not record.protocol_hint: errors.append("udp_probe requires protocol_hint")
        if record.max_response_bytes is None: errors.append("udp_probe requires max_response_bytes")
        if record.probe_payload_hex is None: errors.append("udp_probe requires probe_payload_hex")
    elif record.kind == "os_fingerprint":
        if not record.fingerprint_name: errors.append("os_fingerprint requires fingerprint_name")
        if not record.os_family: errors.append("os_fingerprint requires os_family")
        if not record.os_features: errors.append("os_fingerprint requires os_features")
    elif record.kind == "web_fingerprint":
        if not record.matcher_type or not record.matcher_expression: errors.append("web_fingerprint requires matcher_type and matcher_expression")
        if not record.evidence_dimension: errors.append("web_fingerprint requires evidence_requirements evidence dimension")
        if not record.evidence_requirements: errors.append("web_fingerprint requires evidence_requirements")
    elif record.kind == "device_fingerprint":
        if not record.matcher_type or not record.matcher_expression: errors.append("device_fingerprint requires matcher_type and matcher_expression")
        if not record.device_type and not record.product: errors.append("device_fingerprint requires device_type or product")
    elif record.kind == "port_registry":
        if not record.service: errors.append("port_registry requires service")
        if not record.ports: errors.append("port_registry requires at least one port")
    elif record.kind == "product_record":
        if not record.product: errors.append("product_record requires product")
    elif record.kind == "product_alias":
        if not record.product or not record.alias: errors.append("product_alias requires product and alias")
    elif record.kind == "cpe_record":
        if not record.cpe: errors.append("cpe_record requires at least one CPE 2.3 name")
