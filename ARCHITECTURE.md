...
Normal output is composed by `TerminalReportRenderer` from header, host, port-table, summary, and footer components over the same canonical report. `TerminalCapabilities` is detected once at the CLI boundary and carried in `OutputContext`; renderers never read mutable terminal environment state. Width policy is deterministic: plain below 64 columns or for non-TTY output, narrow at 64–87, medium at 88–119, and wide from 120. `TerminalTheme` applies fixed semantic styles only after layout, while shared core text-safety primitives validate UTF-8, enforce XML 1.0 character ranges, and remove control and bidirectional terminal hazards. Display truncation is bounded by both terminal cells and encoded bytes, so zero-width combining input cannot amplify a narrow row. Redirected/file normal output remains ASCII and keeps stable Nmap-style host and port rows.
...
The CLI may attach `TerminalProgressRenderer` as a presentation-only event sink when normal output is going to an interactive stdout and stderr is also interactive. It uses fixed labels and completed-result counters, throttles repeated batch updates, clears before the Output stage serializes the report, and never computes rate or ETA from post-stage events. File/machine output, one-sided redirection, `TERM=dumb`, and debug logging disable progress. Terminal diagnostics sanitize dynamic text, and the logger atomically consumes the active-progress marker before clearing the transient line and writing a diagnostic, preventing persistent mixed progress/log lines. Observer failures remain isolated by `ScanSession::emit()` and cannot change scan state.
