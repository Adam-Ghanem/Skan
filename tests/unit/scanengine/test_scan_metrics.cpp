...
int main()
{
    ...
    metrics.record_timeout_backoff();
    ...
    assert(metrics.probes_cancelled == 1U);
    assert(metrics.probes_retried == 1U);
    assert(metrics.targets_failed == 1U);
    assert(metrics.retries == 1U);
    assert(metrics.timeout_backoffs == 1U);
    ...
    assert(metrics.srtt_ms.has_value());
    assert(metrics.rttvar_ms.has_value());
    assert(metrics.rto_ms.has_value());
    assert(*metrics.rto_ms >= *metrics.srtt_ms);
