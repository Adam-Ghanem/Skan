...
docker run --rm --volume "$PWD/dist:/dist:ro" \
  ...
  lintian --fail-on error /dist/skan_0.1.2-1_amd64.deb

bash scripts/test_deb_package.sh \
  dist/skan_0.1.2-1_amd64.deb debian:12
bash scripts/test_deb_package.sh \
  dist/skan_0.1.2-1_amd64.deb ubuntu:24.04
```
